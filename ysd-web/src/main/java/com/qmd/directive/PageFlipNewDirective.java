 ...
 *
 */
@Component("pageFlipNewDirective")
public class PageFlipNewDirective implements TemplateDirectiveModel {
	
	public static final String TAG_NAME = "pageFlipNew";
	// 传入参数
	...
	public void execute(Environment env, Map params, TemplateModel[] loopVars, TemplateDirectiveBody body) throws TemplateException, IOException {
		...
		if (pager != null) {
			...
			if (pageCount <= 5) {// 总页数小于5，显示全部
				startPageItemNumber = 1;
				endPageItemNumber = pageCount;
			} else if(pageNumber <=4){// 当前页为头部4条，显示前面5条
				startPageItemNumber = 1;
				endPageItemNumber = 5;
			} else if(pageCount - pageNumber <4){// 当前页为尾部4条，显示最后5条
