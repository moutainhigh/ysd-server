	...
	public synchronized int borrowInvestDo(User user, BorrowTender borrowTender, String ip, int orderNum) {
		...
		if (Double.valueOf(borrowTender.getContinueAmount()) > 0) {
			...
			UserAccountDetail userAccountDetail = AccountDetailUtil.fillUserAccountDetail("tender_continued",
					...
					"续投冻结资金[" + CommonUtil.fillBorrowUrl(borrow.getId(), borrow.getName()) + "]", ip, userAccount);
		...
		if ("1".equals(borrow.getAward())) {
			...
			UserAccountDetail userAccountDetail5 = AccountDetailUtil.fillUserAccountDetail("award_lower", reward, 10000,
					"扣除借款[" + CommonUtil.fillBorrowUrl(borrow.getId(), borrow.getName()) + "]的奖励", ip, boruserAccount);
		...
		if (borrowTender.getContinueAmount() != null && !"".equals(borrowTender.getContinueAmount().trim())
			...
			// BigDecimal
			// continueMoney=BigDecimal.valueOf(Double.valueOf(borrowTender.getContinueAmount()));//获取续投金额
			// 续投奖励=续投金额 * 续投奖励率 * 投资天数
			Listing listing2 = listingDao.getListing(ConstantUtil.CONTINUE_REWARD_WANDER);
			BigDecimal getaward = BigDecimal.valueOf(Double.parseDouble(borrowTender.getContinueAmount())
					* Double.parseDouble(listing2.getKeyValue()) * borrowDay.intValue());
