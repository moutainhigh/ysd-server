 ...
 * 
 */
@Repository("borrowFlowService")
public class BorrowFlowFullServiceImpl extends BaseServiceImpl<BorrowTender, Integer> implements BorrowFlowFullService {
	Logger log = Logger.getLogger(BorrowFlowFullServiceImpl.class);
	...
	public synchronized int borrowInvestDo(User user, BorrowTender borrowTender, String ip) {
		...
		int repayTime = Integer.parseInt(borrow.getTimeLimit());
		...
		// 修改实际投标金额
		// String interest =
		// String.valueOf((Double.valueOf(borrowTender.getAccount()) *
		// borrow.getApr()/100));
		String interest = CommonUtil.setPriceScale(CommonUtil.getInterest(Double.valueOf(borrowTender.getAccount()), borrowDay.intValue(),
				borrow.getApr()));
		...
		if ("1".equals(borrow.getAward())) {
			...
			reward = CommonUtil.setPriceScale(BigDecimal.valueOf(reward.doubleValue() * (borrowDay - 1)));
		}
		...
		if (Double.valueOf(borrowTender.getContinueAmount()) > 0) {
			// 修改用户续单余额
			userAccount.setContinueTotal(BigDecimal.valueOf(userAccount.getContinueTotal().doubleValue()
					- Double.parseDouble(borrowTender.getContinueAmount())));
			// 修改用户冻结金额
			userAccount.setUnableMoney(BigDecimal.valueOf(userAccount.getUnableMoney().doubleValue()
					+ Double.parseDouble(borrowTender.getContinueAmount())));
			...
			UserAccountDetail userAccountDetail = AccountDetailUtil.fillUserAccountDetail("tender_continued",
					BigDecimal.valueOf(Double.valueOf(borrowTender.getContinueAmount())), 10000,
					"续单冻结资金[" + CommonUtil.fillBorrowUrl(borrow.getId(), borrow.getName()) + "]", ip, userAccount);
		...
		if (Double.valueOf(borrowTender.getAbleAmount()) > 0) {
			...
			// 修改用户冻结金额
			userAccount.setUnableMoney(BigDecimal.valueOf(userAccount.getUnableMoney().doubleValue()
					+ Double.parseDouble(borrowTender.getAbleAmount())));
			...
			UserAccountDetail userAccountDetail = AccountDetailUtil.fillUserAccountDetail("tender",
					BigDecimal.valueOf(Double.valueOf(borrowTender.getAbleAmount())), 10000,
					"投标冻结资金[" + CommonUtil.fillBorrowUrl(borrow.getId(), borrow.getName()) + "]", ip, userAccount);
			this.userAccountDetailDao.save(userAccountDetail);
		}
		...
		userRepaymentDetail.setWaitInterest(userRepaymentDetail.getInterest());
		userRepaymentDetail.setWaitAccount(userRepaymentDetail.getAccount());
		...
		userRepaymentDetail.setBorrowDivides(1);
		...
		// 利息管理费
		BigDecimal userserviceCharge = CommonUtil.setPriceScale(
				BigDecimal.valueOf((Double.valueOf(userRepaymentDetail.getInterest()).doubleValue() * Double.valueOf(listing_lixi.getKeyValue()))),
				ConstantUtil.RoundType.roundHalfUp);
		userRepaymentDetail.setServiceCharge(String.valueOf(userserviceCharge));
		userRepaymentDetail.setProfit(String.valueOf(Double.valueOf(userRepaymentDetail.getInterest())
				- Double.valueOf(userRepaymentDetail.getServiceCharge())));

		userRepaymentDetail.setRepaymentYesaccount(userRepaymentDetail.getAccount());
		userRepaymentDetail.setRepaymentYesinterest(userRepaymentDetail.getInterest());
		...
		// 冻结金额 = 冻结金额-投资金额
		userAccount.setUnableMoney(userAccount.getUnableMoney().subtract(tenderCapital));
		// 投资人待收本金 增加
		userAccount.setInvestorCollectionCapital(userAccount.getInvestorCollectionCapital().add(tenderCapital));
		// 投资人待收利息 增加
		userAccount.setInvestorCollectionInterest(userAccount.getInvestorCollectionInterest().add(tenderInterest));

		this.userAccountDao.updateAll(userAccount);

		// 资金记录
		UserAccountDetail userAccountDetail2 = AccountDetailUtil.fillUserAccountDetail("invest",
				BigDecimal.valueOf(Double.valueOf(borrowTender.getAccount())), 10000,
				"投标成功费用扣除[" + CommonUtil.fillBorrowUrl(borrow.getId(), borrow.getName()) + "]", ip, userAccount);
		...
		if ("1".equals(borrow.getAward())) {
			...
			if (0 == borrow1.getAwardType()) {
				...
				userAccount.setAbleMoney(CommonUtil.setPriceScale(userAccount.getAbleMoney().add(reward)));

				userAccountDao.updateAll(userAccount);// 【完成修改投资人账户】

				// 资金记录
				UserAccountDetail userAccountDetail11 = AccountDetailUtil.fillUserAccountDetail("award_add", reward, 10000,
						"借款[" + CommonUtil.fillBorrowUrl(borrow.getId(), borrow.getName()) + "]的奖励", ip, userAccount);
				...
				userAccountDao.updateAll(userAccount);// 【完成修改投资人账户】
				// 资金记录
				// UserAccountDetail userAccountDetail11 =
				// AccountDetailUtil.fillUserAccountDetail("award_toubiao",
				// reward,
				// 10000,
				// "借款["+CommonUtil.fillBorrowUrl(borrow.getId(),
				// borrow.getName())
				// + "]的红包奖励",
				// ip, userAccount);
				//
		...
		// 设置债权转让信息
		// fillInsertZqzrDate(borrow,borrowTender, userRepaymentDetail);

		// ------增加借款人账户-------------------------------------

		UserAccount boruserAccount = this.userAccountDao.getUserAccountByUserId(borrow.getUserId());
		boruserAccount.setTotal(boruserAccount.getTotal().add(tenderCapital));
		// 可用余额 = 原可用金额 + 本次的待还本金
		...
		UserAccountDetail userAccountDetail3 = AccountDetailUtil.fillUserAccountDetail("borrow_success",
				BigDecimal.valueOf(Double.valueOf(borrowTender.getAccount())), 10000,
				"对标[" + CommonUtil.fillBorrowUrl(borrow.getId(), borrow.getName()) + "]借款成功", ip, boruserAccount);
		...
		// 手续费 = 百分比/100 * 借款金额
		double deduction = (Double.parseDouble(listing.getKeyValue())) * (tenderCapital.doubleValue());
		// 账户总金额，账户可用金额 ：扣去借款手续费
		boruserAccount.setAbleMoney(boruserAccount.getAbleMoney().subtract(BigDecimal.valueOf(deduction)));
		boruserAccount.setTotal(boruserAccount.getTotal().subtract(BigDecimal.valueOf(deduction)));
		...
		UserAccountDetail userAccountDetail4 = AccountDetailUtil.fillUserAccountDetail("borrow_fee", BigDecimal.valueOf(deduction), 10000, "对标["
				+ CommonUtil.fillBorrowUrl(borrow.getId(), borrow.getName()) + "]借款手续费", ip, boruserAccount);
		...
		if ("1".equals(borrow.getAward())) {
			// 扣除借款人的奖金
			if (0 == borrow1.getAwardType()) {
				boruserAccount.setAbleMoney(boruserAccount.getAbleMoney().subtract(reward));
				boruserAccount.setTotal(boruserAccount.getTotal().subtract(reward));

				// boruserAccount.setCashMoneyFrozen(boruserAccount.getCashMoneyFrozen().subtract(reward));
				userAccountDao.updateAll(boruserAccount);

				UserAccountDetail userAccountDetail5 = AccountDetailUtil.fillUserAccountDetail("award_lower", reward, 10000,
						"扣除借款[" + CommonUtil.fillBorrowUrl(borrow.getId(), borrow.getName()) + "]的" + str, ip, boruserAccount);
		...
		// ★★★★投资人续单金额奖励 开始★★★★
		if (borrowTender.getContinueAmount() != null && !"".equals(borrowTender.getContinueAmount().trim())
				&& Double.valueOf(borrowTender.getContinueAmount()) > 0) {
			// BigDecimal
			// continueMoney=BigDecimal.valueOf(Double.valueOf(borrowTender.getContinueAmount()));//获取续单金额
			// 续单奖励=续单金额 * 续单奖励率
			// Listing listing2 =
			// listingDao.getListing(ConstantUtil.CONTINUE_REWARD_WANDER);
			// BigDecimal
			// getaward=BigDecimal.valueOf(Double.parseDouble(borrowTender.getContinueAmount())*Double.parseDouble(listing2.getKeyValue())*Integer.parseInt(borrow.getTimeLimit()));
			// BigDecimal
			// getaward=BigDecimal.valueOf(Double.parseDouble(borrowTender.getContinueAmount())*Double.parseDouble(listing2.getKeyValue()));
			...
			UserAccountDetail userAccountDetail6 = AccountDetailUtil.fillUserAccountDetail("award_continued", getaward, 10000,
		...
		if (listing_zhuan.getIsEnabled()) {
			bonus = CommonUtil.mul(
					BigDecimal.valueOf(Double.parseDouble(listing_zhuan.getKeyValue()) * Double.parseDouble(borrowTender.getAccount())),
					BigDecimal.valueOf(Double.parseDouble(borrow.getTimeLimit())));
			...
			if (bonus.compareTo(new BigDecimal(0)) == 1) {
				...
				this.userAccountDao.update(userAccount);
				// UserAccountDetail userAccountDetail1 =
				// AccountDetailUtil.fillUserAccountDetail(
				// "money_for_bonus",bonus,user.getId(),
				// "系统将标"+borrow.getName()+"获得的"+bonus+"红包变成可用金额",ip,
				// userAccount);
				//
		...
		if (listing_tui.getIsEnabled()) {
			...
			tuijian = CommonUtil.mul(
					BigDecimal.valueOf(Double.parseDouble(listing_tui.getKeyValue()) * Double.parseDouble(borrowTender.getAccount())),
					BigDecimal.valueOf(Double.parseDouble(borrow.getTimeLimit())));
			...
			UserAccountDetail userAccountDetail1 = AccountDetailUtil.fillUserAccountDetail("tui_detail_award", bonus, user.getTgOneLevelUserId(),
					"获得好友" + user.getUsername() + "投资标" + borrow.getName() + "后得到奖励" + bonus, ip, userAccTui);
