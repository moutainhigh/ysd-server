...
public class UserServiceImpl extends BaseServiceImpl<User, Integer> implements UserService {
	...
	public String buildRecoverKey() {
		return System.currentTimeMillis() + ConstantUtil.RECOVER_KEY_SEPARATOR + CommonUtil.getUUID()
				+ DigestUtils.md5Hex(String.valueOf(Math.random() * 1000000000));
	}
	...
	public void addUser(User user) {
		...
		if(user.getPlaceChilderId() != null){
			...
			analys.setPlaceName(pc.getPlaceName());
		}
	...
	/**
	 * public void hongBaoRegisterJiLu(Hongbao hongbao ,UserAccount
	 * userAccount,User user,Integer day_qixian){ for(int
	 * i=0;i<hongbao.getTenth();i++){ BigDecimal award0 = new BigDecimal(10);
	 * UserAccount userAccount0 =
	 * userAccountDao.getForUpdate(userAccount.getId(), userAccount);
	 * userAccount0.setAwardMoney(userAccount0.getAwardMoney().add(award0));
	 * userAccountDao.update(userAccount0);
	 * 
	 * //添加红包记录 UserHongbao hb = new UserHongbao(); hb.setUserId(user.getId());
	 * hb.setHbNo(CommonUtil.getDate2String(new Date(),
	 * "yyyyMMdd")+CommonUtil.getRandomString(5)); hb.setName("用户注册");
	 * hb.setMoney(award0); hb.setUsedMoney(BigDecimal.ZERO); hb.setStatus(0);
	 * Date d = new Date(); hb.setStartTime(d);
	 * hb.setEndTime(CommonUtil.date2end(CommonUtil.getDateAfter(d,
	 * day_qixian))); hb.setSource(1); hb.setSourceString("用户注册");
	 * hb.setSourceUserId(user.getId()); hb.setSourceBorrowId(null);
	 * hb.setUsedBorrowId(null); userHongbaoDao.save(hb); // 奖励账户资金记录
	 * UserAwardDetail uad = new UserAwardDetail();
	 * uad.setUserId(user.getId());// 用户ID
	 * uad.setType(ConstantUtil.MONEY_LOG_REGISTER_AWARD);// 类型（同资金记录）
	 * uad.setMoney(award0);// 操作金额 uad.setSignFlg(1);
	 * uad.setAwardMoney(userAccount0.getAwardMoney());// 奖励账户
	 * uad.setRemark("用户注册奖励");// 备注 uad.setReserve1(hb.getHbNo());
	 * uad.setRelateKey("hongbao_id"); userAwardDetailDao.save(uad); }
	 * 
	 * for(int i=0;i<hongbao.getTwentieth();i++){ BigDecimal award0 = new
	 * BigDecimal(20); UserAccount userAccount0 =
	 * userAccountDao.getForUpdate(userAccount.getId(), userAccount);
	 * userAccount0.setAwardMoney(userAccount0.getAwardMoney().add(award0));
	 * userAccountDao.update(userAccount0);
	 * 
	 ...
	 * day_qixian))); hb.setSource(1); hb.setSourceString("用户注册");
	 * hb.setSourceUserId(user.getId()); hb.setSourceBorrowId(null);
	 * hb.setUsedBorrowId(null); userHongbaoDao.save(hb);
	 * 
	 * // 奖励账户资金记录 UserAwardDetail uad = new UserAwardDetail();
	 * uad.setUserId(user.getId());// 用户ID
	 * uad.setType(ConstantUtil.MONEY_LOG_REGISTER_AWARD);// 类型（同资金记录）
	 * uad.setMoney(award0);// 操作金额 uad.setSignFlg(1);
	 * uad.setAwardMoney(userAccount0.getAwardMoney());// 奖励账户
	 ...
	 * uad.setRelateKey("hongbao_id"); userAwardDetailDao.save(uad); }
	 * 
	 * for(int i=0;i<hongbao.getFiftieth();i++){ BigDecimal award0 = new
	 * BigDecimal(50); UserAccount userAccount0 =
	 * userAccountDao.getForUpdate(userAccount.getId(), userAccount);
	 * userAccount0.setAwardMoney(userAccount0.getAwardMoney().add(award0));
	 * userAccountDao.update(userAccount0);
	 * 
	...
	public void updateUserByLoginSuccess(User user) {
		...
		if(user.getPlaceChilderId() != null){
			PlaceChilder pc = placeChilderDao.get(user.getPlaceChilderId());
			if(pc != null){
				pc.setLoginNum(pc.getLoginNum()+1);
				placeChilderDao.update(pc);
			}
		}
	...
	public void updateEmail_tg(User user, String ip) {
	}
		...
		if (user.getTgStatus() != null && user.getTgStatus() == 0) {// 推广奖励没发
			if (user.getTgOneLevelUserId() != null) {// 存在上级好友
				if (user.getRealStatus() != null && user.getRealStatus() == 1) {// 实名已经通过
					...
					if (hbao.getIsEnabled().compareTo(1) == 0 && hbao.getTotal().compareTo(BigDecimal.ZERO) > 0) {
						// Integer day_qixian = Integer.parseInt(
						// listingDao.getKeyValue("tuijian_award2"));//红包有效期
						// hongBaoRealNameJiLu(hb,user,day_qixian);
...
/**
	public void hongBaoRealNameJiLu(Hongbao hongbao, User user, Integer day_qixian) {
...
**/
	...
	public Boolean isPassword(String username, String password, String passwordType) {
		...
		String pwd = "";
		if (passwordType.equals("0")) {// 登录密码aafbfd18b9c41cc0ccbb481e2ccab579
	...
	public void updateTenderAuto(User user) {
		...
		// if (dateFlg == false && user.getAutoTenderLimitBegin() != null &&
		// !user.getAutoTenderLimitBegin().equals(oldUser.getAutoTenderLimitBegin()))
		// {
		// dateFlg = true;
		// }
		// if (dateFlg == false && user.getAutoTenderLimitEnd() != null &&
		// !user.getAutoTenderLimitEnd().equals(oldUser.getAutoTenderLimitEnd()))
		// {
		// dateFlg = true;
		// }
		// if (dateFlg == false && user.getAutoTenderRepayWay() != null &&
		// !user.getAutoTenderRepayWay().equals(oldUser.getAutoTenderRepayWay()))
		// {
		// dateFlg = true;
		// }
		if (dateFlg) {
	...
	public int authRealName(User user) {
		...
		try {
			...
			Map<String, String> map = QueryCard.getParseMapFromXMLStream(document, "/data/message");
			...
			if ("0".equals(map.get("status"))) {
				map = QueryCard.getParseMapFromXMLStream(document, "/data/policeCheckInfos/policeCheckInfo/message");
				l.setPoliceStatus(map.get("status"));
				l.setPoliceValue(map.get("value"));

				if ("0".equals(map.get("status"))) {
					map = QueryCard.getParseMapFromXMLStream(document, "/data/policeCheckInfos/policeCheckInfo");
					...
					if ("3".equals(map.get("compStatus")) && "一致".equals(map.get("compResult"))) {
