    ...
    <div id="main">
        ...
        <div class="content fr">
            ...
            <div class="recharge">
                 ...
                 <form id="rechargeForm" method="post" action="/payment/submitRecharge.do">
              <!-- <form id="rechargeForm" method="post" action="/payment/submit.do" target="_blank">-->
                <input type="hidden" name="id" value="${loginUser.id}"/>
                <input type='hidden' name='idcard' value='${loginUser.realStatus}'/>
                   
                    <div class="account_balance">
                        账户余额： <span>${ableMoney}</span>元
                    </div>
                    <div class="recharge_value">
                        充值金额： 
          <input type="text" name="userAccountRecharge.money" id="money" onkeyup="this.value=this.value.replace(/[^0-9\.]/g,'');"   placeholder="请输入充值金额" value="${money}"/>&nbsp;&nbsp;元        
                    <p></p>
                    </div>
      ...
      <input type="text"  placeholder="请输入验证码" name="mycode" onfocus="verifyCode()" id="mycode"> <a  onclick="verifyCodeLink();" title="点击更换"><img  src="${base}/rand.do" id="code_img"></a>
                ...
                <div class='recharge_ts'><span class='recharge_tb'></span>温馨提示：<span>快捷支付！</span></div>
				<!--<div class='recharge_ts'><span class='recharge_tb'></span>温馨提示：<span>绑定银行卡后才能使用快捷支付及提现功能，<a href='${base}/userCenter/toBank.do'>立即绑卡</a></span></div>-->
            </div>
        </div>
