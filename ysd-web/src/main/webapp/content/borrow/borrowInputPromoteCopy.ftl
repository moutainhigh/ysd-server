...
<div style="width:670px; float:right; padding:0px 15px 0px 18px; ">
      <div style="padding-top:30px;">
        ...
        <a href="${base}/borrow/borrowChoose.do" style="color:#646464;font-family:'宋体';">我要借款</a>
      </div>
      <div style=" width:661px;background:#d4d4d4; height:42px; line-height:42px; padding-left:9px; margin-top:8px; float:left;">
        <a  class="hr hre">新增借款</a>
      </div>

	<form id="form1" name="form1" method="post" action="${base}/borrow/saveBorrowPromote.do"  onsubmit="return check_form_promote();" enctype="multipart/form-data" >
		 <input type="hidden" name="borrow.parentFlg" value="2" />
		 <input type="hidden" name="borrow.parentId" value="${borrow.id}" />
         ...
         <input type="hidden" id="borrow_childMoneyWait" value="${borrow.childMoneyWait}"/>
		...
		<div class="base_frm" style="padding-top:30px;padding-bottom:30px; background:#fff;">
					<table >
						<tbody>
							...
							<tr>
								...
								<td style=""><input type="text" id="borrowname" value="${borrow.name}" name="borrow.name" style="width:284px; height:30px; line-height:30px; padding-left:4px; border:1px solid #c6c6c6; border-radius:5px;"/></td>
							</tr><tr height="20"><td></td><td></td></tr>
							<tr>
								<td  style="width:90px; padding-right:5px; text-align:right;color:#646464;font-family:'宋体';">续标：</td>
								<td style="">项目总额：${borrow.parentMoney} 已发：${borrow.childMoneyFinish} 待发：${borrow.childMoneyReady} 未发：${borrow.childMoneyWait}</td>
							</tr><tr height="20"><td></td><td></td></tr>
							<tr>
								...
								<td class="base_data-list" valign="top">
									...
									<#list borrowImgList as tem>
										<tr id="trid_${tem_index}">
											<td>  标题：<input type="text" style="width:90px; height:25px; line-height:30px; padding-left:4px; border:1px solid #c6c6c6; border-radius:5px;"  name="vouchersTitle" value="${tem.name}">  
											<input type="file" style="width:165px; height:25px;"  id="imageItem_${tem_index}" name="imageFile">  
											<input type="hidden" id="voucher_${tem_index}" name="vouchers" value="${tem.url}">
											</td>
											<td><input type="button" style=" width:44px;" class="lsc lscr_o1" onclick="tempback(${tem_index});ajaxFileUploadImageWithRtn('imageItem_${tem_index}','/file/ajaxUploadImage.do','voucher_${tem_index}',uploadFileHideBack);" id="btnUpload_${tem_index}" value="上传">	
											<input type="button" onclick="moveOut('${tem_index}')" value="删除" style=" width:44px;" class="lsc lscr_o1">	
											<input type="button" onclick="moveUp('${tem_index}')" value="上移" style=" width:44px;" class="lsc lscr_o1">	
											<input type="button" onclick="moveDown('${tem_index}')" value="下移" style=" width:44px;" class="lsc lscr_o1">	
											<span id="idlook_${tem_index}">
												<a target="_blank" href="${base}/index.img?image=${tem.url}" style=" width:44px;" class="lsc lscr_o1">查看</a>
											</span>
					...
					<table >
						<tbody>
							...
							<tr>
								...
								<td style="">
								<input type="checkbox"  name="award1" id="award1" onclick="change_award(1)" <#if borrow.award==1||borrow.award==2>checked</#if> value="1" />是否启用，
								...
								<input type="text" id="funds" name="borrow.funds" <#if borrow.award==1||borrow.award==2>value="${borrow.funds}"<#else>disabled value=""</#if>  onkeyup="clearNoNum(this)" size="10" style="width:284px; height:30px; line-height:30px; padding-left:4px; border:1px solid #c6c6c6; border-radius:5px;"/>
								</td>
							</tr><tr height="20"><td></td><td></td></tr>
							
							<tr>
								<td  style="width:90px; padding-right:5px; text-align:right;color:#646464;font-family:'宋体';">发放奖励设置励：</td>
								<td style="">
								<input type="checkbox"  name="award2" id="award2" onclick="change_award(2)" value="2" <#if borrow.award==2>checked</#if> />是否启用还款完成时发放奖励
								</td>
							</tr><tr height="20"><td></td><td></td></tr>
							...
							<tr>
								...
								<td style="">
									<input type="text" name ="borrow.timeLimit" value="${borrow.timeLimit}" id="timeLimit" onblur="loadRepaymentPlan();" onkeyup="value=value.replace(/[^0-9]/g,'')"  style="width:284px; height:30px; line-height:30px; padding-left:4px; border:1px solid #c6c6c6; border-radius:5px;"/>
							...
							<tr>
								...
								<td style="">
									<input type="checkbox" name="isDxb1" id="isDxb1" onclick="change_isDxb(1)" <#if borrow.isDxb==1>checked</#if>  value="1"/>&nbsp;&nbsp;是，定向密码为：&nbsp;&nbsp;<input type="text" id="pwd" name="borrow.pwd" <#if borrow.isDxb==1><#else>disabled</#if>   value="" size="10"style="width:284px; height:30px; line-height:30px; padding-left:4px; border:1px solid #c6c6c6; border-radius:5px;"/>
			...
			<div class="base_frm" style="padding-top:10px;padding-bottom:10px;background:#fff;">
					<table >
						<tbody>
							<tr>
								<td  style="width:90px; padding-right:5px; text-align:right;color:#646464;font-family:'宋体';" width="120">交易密码：</td>
...
$(function(){
	...
	if ($("#borrow_lowestAccount").val()!='') {
		$("#lowestAccount").val($("#borrow_lowestAccount").val());
	}
	if ($("#borrow_mostAccount").val()!='') {
		$("#mostAccount").val($("#borrow_mostAccount").val());
	}
	if ($("#borrow_validTime").val()!='') {
		$("#validTime").val($("#borrow_validTime").val());
	}
	if ($("#borrow_divides").val()!='') {
		$("#borrowDivides").val($("#borrow_divides").val());
	}
	if ($("#borrow_style").val()!='') {
		$("#borrowStyle").val($("#borrow_style").val());
	}
	
<#if borrowIsday==0>
	commit_year2month('apr','monthapr');
<#elseif borrowIsday==1>
	commit_day2year('apr','yearapr');
</#if>
...
function addImgItem() {
	...
	str += '  <input type="file" name="imageFile" style="width:170px; height:25px;"  id="imageItem_'+x+'" />';
	str += '  <input type="hidden" name="vouchers" id="voucher_'+x+'" />';
	str += '</td>';
    str += '<td>';
    str += '	<input type="button" value="上传" style=" width:46px;" class="lsc lscr_o1"  id="btnUpload_'+x+'"  onclick="tempback('+x+');ajaxFileUploadImageWithRtn(\'imageItem_'+x+'\',\'${base}/file/ajaxUploadImage.do\',\'voucher_'+x+'\',uploadFileHideBack);" />';
    str += '	<input type="button" value="删除" onclick="moveOut(\''+x+'\')" style=" width:46px;" class="lsc lscr_o1" />';
    str += '	<input type="button" value="上移" onclick="moveUp(\''+x+'\')"  style=" width:46px;" class="lsc lscr_o1"/>';
    str += '	<input type="button" value="下移" onclick="moveDown(\''+x+'\')"  style=" width:46px;" class="lsc lscr_o1"/>';
    str += '	<span id="idlook_'+x+'">&nbsp;</span></td>';
    str += '</tr>';
...
function uploadFileHideBack() {
	alert("上传成功！"+tback);
	var str ='';
	var mg =$("#voucher_"+tback).val();
	str ='<a href="${base}/index.img?image='+mg+'" target="_blank">查看</a>'
	$("#idlook_"+tback).html(str);
}

function showChildMoneyWait() {
	var wt =parseInt($("#borrow_childMoneyWait").val());
	var ac = $("#account").val();
	var nac = 0;
	if (ac !='') {
		nac=parseInt(ac);
