...
<html>
 <head>
 <meta charset="gb2312" />
  <title>${Application ["qmd.setting.name"]}—国资控股|专业、安全、透明的互联网金融服务平台-我的账户-已收明细</title>
...
<div class="c_right">
				 ...
				 <div class="rightwrap">
				      <div class="rightcon">
						<ul class="wtitle_l">
                             <li><a name="" href="../borrow/userTenderBorrowList.do?bStatus=3,4">正在收款</a></li>
                             <li><a name="" href="${base}/borrow/uncollectedDetailList.do">未收明细</a></li>
                             <li><a class="wtitle_lNa " name="" href="">已收明细</a></li>
					...
					-->
                         <form id="listForm" method="post" action="receivedDetailList.do" >
...
-->
                                   ...
                                   <#list pager.result as borrowTemp>
									   ...
									   <td>${borrowTemp.borrowPeriods}/${borrowTemp.timeLimit}</td>
									   <td>${borrowTemp.repaymentAccount}</td>
									   <td>${borrowTemp.waitAccount}</td>
									   <td>${borrowTemp.waitInterest}</td>
<#--
									   <td>${borrowTemp.lateInterest?string.currency}</td>
									   <td>${borrowTemp.lateDays}</td>
									   <td>
									   		<#if borrowTemp.status =1 >
									   			已还
									   		</#if>
									   	</td>
-->
									  ...
									  <!--page-->
										<@pagination pager=pager baseUrl="/borrow/receivedDetailList.do"  parameterMap = parameterMap>
