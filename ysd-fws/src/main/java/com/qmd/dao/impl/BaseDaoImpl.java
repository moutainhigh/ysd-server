...
public class BaseDaoImpl<T, PK extends Serializable> extends
	...
	public T baseLoad(PK id) {
		return this.getSqlSession().selectOne(
				getClassNameSpace() + ".baseLoad", id);
	}

	public void baseUpdate(T entity) {
		this.getSqlSession().update(getNameSpace(entity) + ".baseUpdate",
				entity);
	}
	...
	public T getForUpdate(PK id, T entity) {
		...
		return this.getSqlSession().selectOne(
				getNameSpace(entity) + ".getForUpdate", id);
	}
	...
	public T getById(PK id, T entity) {
		...
		return this.getSqlSession().selectOne(
				getNameSpace(entity) + ".getById", id);
	}
	@Override
	public T getById(PK id) {
		return this.getSqlSession().selectOne(
				getClassNameSpace() + ".getById", id);
	}
	...
	// }
	/**
	 * @SuppressWarnings("unchecked") public T get(PK id) { Assert.notNull(id,
	 *                                "id is required"); return (T)
	 *                                getSession().get(entityClass, id); }
	 * @SuppressWarnings("unchecked") public T load(PK id) { Assert.notNull(id,
	 *                                "id is required"); return (T)
	 *                                getSession().load(entityClass, id); }
	 * @SuppressWarnings("unchecked") public List<T> getAllList() {
	 *                                ClassMetadata classMetadata =
	 *                                sessionFactory
	 *                                .getClassMetadata(entityClass); String
	 *                                hql; if
	 *                                (ArrayUtils.contains(classMetadata.
	 *                                getPropertyNames(),
	 *                                ORDER_LIST_PROPERTY_NAME)) { hql = "from "
	 *                                + entityClass.getName() +
	 *                                " as entity order by entity." +
	 *                                ORDER_LIST_PROPERTY_NAME + " desc"; } else
	 ...
	 *                                "select count(*) from " +
	 *                                entityClass.getName(); return (Long)
	 *                                getSession
	 *                                ().createQuery(hql).uniqueResult(); }
	 * @SuppressWarnings("unchecked") public PK save(T entity) {
	 *                                Assert.notNull(entity,
	 *                                "entity is required"); if (entity
	 *                                instanceof BaseEntity) { try { Method
	 *                                method =
	 *                                entity.getClass().getMethod(BaseEntity
	 *                                .ON_SAVE_METHOD_NAME);
	 *                                method.invoke(entity); return (PK)
	 *                                getSession().save(entity); } catch
	 *                                (Exception e) { e.printStackTrace();
	 *                                return null; } } else { return (PK)
	 ...
	 *                                method =
	 *                                entity.getClass().getMethod(BaseEntity
	 *                                .ON_UPDATE_METHOD_NAME);
	 *                                method.invoke(entity);
	 *                                getSession().update(entity); } catch
	 *                                (Exception e) { e.printStackTrace(); } }
	 ...
	 * @SuppressWarnings("unchecked") public void delete(PK id) {
	 *                                Assert.notNull(id, "id is required"); T
	 *                                entity = (T)
	 *                                getSession().load(entityClass, id);
	 *                                getSession().delete(entity); }
	 * @SuppressWarnings("unchecked") public void delete(PK[] ids) {
	 *                                Assert.notEmpty(ids,
	 *                                "ids must not be empty"); for (PK id :
	 *                                ids) { T entity = (T)
	 *                                getSession().load(entityClass, id);
	 *                                getSession().delete(entity); } }
	 * 
	 ...
	 *                                findPager(pager, criteria); }
	 * 
	 *                                public Pager findPager(Pager pager,
	 *                                Order... orders) { Criteria criteria =
	 *                                getSession().createCriteria(entityClass);
	 *                                for (Order order : orders) {
	 *                                criteria.addOrder(order); } return
	 *                                findPager(pager, criteria); }
	 * 
	 *                                public Pager findPager(Pager pager,
	 *                                Criteria criteria) { Assert.notNull(pager,
	 *                                "pager is required");
	 *                                Assert.notNull(criteria,
	 *                                "criteria is required");
	 * 
	 *                                Integer pageNumber =
	 *                                pager.getPageNumber(); Integer pageSize =
	 *                                pager.getPageSize(); String searchBy =
	 *                                pager.getSearchBy(); String keyword =
	 *                                pager.getKeyword(); String orderBy =
	 *                                pager.getOrderBy(); Pager.Order order =
	 *                                pager.getOrder();
	 * 
	 *                                if (StringUtils.isNotEmpty(searchBy) &&
	 *                                StringUtils.isNotEmpty(keyword)) { if
	 *                                (searchBy.contains(".")) { String alias =
	 *                                StringUtils.substringBefore(searchBy,
	 *                                "."); criteria.createAlias(alias, alias);
	 *                                } criteria.add(Restrictions.like(searchBy,
	 *                                "%" + keyword + "%")); }
	 * 
	 *                                pager.setTotalCount(
	 *                                criteriaResultTotalCount(criteria));
	 * 
	 ...
	 *                                ClassMetadata classMetadata =
	 *                                sessionFactory
	 *                                .getClassMetadata(entityClass); if
	 *                                (!StringUtils.equals(orderBy,
	 *                                ORDER_LIST_PROPERTY_NAME) &&
	 *                                ArrayUtils.contains
	 *                                (classMetadata.getPropertyNames(),
	 *                                ORDER_LIST_PROPERTY_NAME)) {
	 *                                criteria.addOrder
	 *                                (Order.asc(ORDER_LIST_PROPERTY_NAME));
	 *                                criteria.addOrder(Order.desc(
	 *                                CREATE_DATE_PROPERTY_NAME)); if
	 *                                (StringUtils.isEmpty(orderBy) || order ==
	 *                                null) {
	 *                                pager.setOrderBy(ORDER_LIST_PROPERTY_NAME
	 *                                ); pager.setOrder(Pager.Order.asc); } }
	 *                                else if (!StringUtils.equals(orderBy,
	 *                                CREATE_DATE_PROPERTY_NAME) &&
	 *                                ArrayUtils.contains
	 *                                (classMetadata.getPropertyNames(),
	 *                                CREATE_DATE_PROPERTY_NAME)) {
	 *                                criteria.addOrder
	 *                                (Order.desc(CREATE_DATE_PROPERTY_NAME));
	 *                                if (StringUtils.isEmpty(orderBy) || order
	 *                                == null) {
	 ...
	 * 
	 *                                pager.setResult(criteria.list()); return
	 *                                pager; }
	 * 
	 *                                // 获取Criteria查询数量
	 * @SuppressWarnings("unchecked") private int
	 *                                criteriaResultTotalCount(Criteria
	 *                                criteria) { Assert.notNull(criteria,
	 *                                "criteria is required");
	 * 
	 *                                int criteriaResultTotalCount = 0; try {
	 ...
	 *                                List<OrderEntry> orderEntries = (List)
	 *                                ReflectionUtil.getFieldValue(criteriaImpl,
	 *                                "orderEntries");
	 *                                ReflectionUtil.setFieldValue(criteriaImpl,
	 *                                "orderEntries", new ArrayList());
	 * 
	 *                                Integer totalCount = ((Long)
	 *                                criteriaImpl.setProjection
	 *                                (Projections.rowCount
	 *                                ()).uniqueResult()).intValue(); if
	 *                                (totalCount != null) {
	 *                                criteriaResultTotalCount = totalCount; }
	 * 
	 *                                criteriaImpl.setProjection(projection); if
	 *                                (projection == null) {
	 *                                criteriaImpl.setResultTransformer
	 *                                (CriteriaSpecification.ROOT_ENTITY); } if
	 *                                (resultTransformer != null) {
	 *                                criteriaImpl.
	 *                                setResultTransformer(resultTransformer); }
	 *                                ReflectionUtil.setFieldValue(criteriaImpl,
	 *                                "orderEntries", orderEntries); } catch
	 *                                (Exception e) {
	 * 
	...
	public <T> BasePager<T> queryPageByObject(T obj, Integer pageSize,
	...
	public T getForUpdate(PK id) {
		...
		return this.getSqlSession().selectOne(
				getClassNameSpace() + ".getForUpdate", id);
	}
}