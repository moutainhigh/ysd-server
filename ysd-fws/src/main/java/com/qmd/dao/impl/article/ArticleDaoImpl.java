...
public class ArticleDaoImpl  extends BaseDaoImpl<Article, Integer>  implements ArticleDao{
	...
	public List<Article> getAgencyMessage(Map<String,Object> map){
		List<Article> articleList = this.getSqlSession().selectList("Article.getAgencyMessage",map);
		return articleList;
	}
	
	public Pager getAgencyMessagePager(Pager pager,Map<String,Object> map){
		Integer count = Integer.parseInt(this.getSqlSession().selectOne("Article.getAgencyMessageCount",map).toString());
		pager.setTotalCount(count);
		map.put("pager", pager);
		List<Article> aList = getAgencyMessage(map);
		pager.setResult(aList);
		return pager;
	}
