...
//			val = val * ConstantUtil.DAYS_OF_YEAR / 10 ;// 天利率（千分）/1000 * 365 * 100 =====显示带百分号的年利率
//		}
//		BigDecimal bd = CommonUtil.setPriceScale(BigDecimal.valueOf(val));
		
		BigDecimal val = BigDecimal.ZERO;
		if (apr != null && !"".equals(apr)) {
			val = new BigDecimal(apr);
			val = val.multiply(BigDecimal.valueOf(ConstantUtil.DAYS_OF_YEAR ).divide(BigDecimal.valueOf(10)));//// 天利率（千分）/1000 * 365 * 100 =====显示带百分号的年利率
		}
		
		BigDecimal bd = CommonUtil.setPriceScale(val);
