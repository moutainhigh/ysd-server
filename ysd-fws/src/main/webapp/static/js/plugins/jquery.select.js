...
$.fn.extend({
	sSelect: function() {
		...
		$(this).before("<span class='dropListBox' id="+selectId+" tabIndex="+tabindex+"></span>").prependTo($("#"+selectId));
		var selectZindex = $(this).css('z-index'),selectIndex = $('#'+selectId+' option').index($('#'+selectId+' option:selected')[0]);
		$('#'+selectId).append('<div class="options"><h4></h4><ul class="list"></ul></div>');
		...
		.bind("click",function(e){
			if($('#'+selectId+' ul').css("display") == 'block'){
				...
				if ($.browser.msie || $.browser.opera){$('.dropListBox').css({'position':'relative','z-index':'0'});}
				$('#'+selectId).css({'position':'relative','z-index':'0'});
