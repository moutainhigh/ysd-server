				...
				<#if borrow_index %2 != 0>
					<#assign flag = "td1">
									...
									<tr height="39">
										...
										<td class="${flag}">￥${borrow.repaymentAccount}</td>
										<td class="${flag}">￥${(borrow.showRepayCapital+borrow.showRepayInterest)}</td>
										<td class="${flag}">￥${(borrow.showPayedCapital+borrow.showPayedInterest)}</td>
										<td class="${flag}"><a href="${base}/borrow/borrowRepayList.do?borrow.id=${borrow.id}">查看详情</a></td>
									</tr>
			</#list>
          </tbody>
          <tfoot>
