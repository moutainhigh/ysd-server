...
<head>
	<meta charset="utf-8" />
	<title>${Application ["qmd.setting.name"]}-项目介绍</title>
						...
						<#list articleList as article>
							<table <#if article_index==0>style="margin-top:10px;"</#if> >
								<tr>
									...
									<td valign="top">
										<#if article.url!>
											<h3><a href="<@article.url?interpret />" class="tc2">${substringWord(article.title, 20, "")}</a><span class="tc3">${article.createDate?string("yyyy-MM-dd")}</span></h3>
										<#else>
											<h3><a href="${base}/article/content/${article.id}.htm" class="tc2">${substringWord(article.title, 20, "")}</a><span class="tc3">${article.createDate?string("yyyy-MM-dd")}</span></h3>
										</#if>	
