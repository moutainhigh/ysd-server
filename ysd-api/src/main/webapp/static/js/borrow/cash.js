function check()
{
		...
		if(typeId==0){
			...
			if(inputValue==0||inputValue==""){
				...
				alert("您好，提现资金不能低于100元高于"+maxCashMoney+"元");
				obj.value=0;
	...
	if (parseFloat(obj.value) > 0)
	{
		...
		if(typeId==0){
			...
			if(ableCashMoney ==0 ){
				...
				alert('您申请提现的金额中包含受15天内充值资金提现规则限制,超出部分将收取(超出额 乘以'+ feeValue+')的提现手续费,还请注意!');
			}
...
function getCashFeeValue(cashAmount){
	...
	var userCashChargeMoney = $("#userCashChargeMoney").val();
...
//		}
	...
	cashFee = changeTwoDecimal(parseFloat(cashFee) + parseFloat(userCashChargeMoney));
	if (cashAmount <= free_cash){
