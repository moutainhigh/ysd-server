...
public class BorrowPromoteServiceImpl extends BaseServiceImpl<BorrowTender,Integer> implements BorrowPromoteService {
	...
	public synchronized int borrowInvestDo(User user,BorrowTender borrowTender,String ip,Integer[] hongbaoId) {
		...
		List<UserAnalys> analysList = userAnalysDao.queryListByMap(detailmap);
		if(analysList != null && analysList.size() >0){
			UserAnalys analys = analysList.get(0);
			
				//IOS回调
				if(StringUtils.isNotEmpty(analys.getIdfa())){
					iosIdfaDao.tenderBack(analys.getIdfa(),1);
				}else if(StringUtils.isNotEmpty(analys.getMac())){
					iosIdfaDao.tenderBack(analys.getMac(),2);
				}
				
				//android回调
				if(analys.getPlaceChilderId() != null && analys.getPlaceChilderId().compareTo(62)==0 &&  StringUtils.isNotEmpty(analys.getImei())){
					iosIdfaDao.tenderBack(analys.getImei(),1);
				}
		}
	...
	**/
	...
	public synchronized int borrowInvestDoHNew(User user,BorrowTender borrowTender,String ip,Integer[] hongbaoId) {
		...
		if(Double.parseDouble(borrowTender.getMoney()) > Double.parseDouble(borrow.getBalance())){
			...
			System.out.println("========用户选择红包的id==============="+ JSONArray.toJSONString(hongbaoId));
			System.out.println("===========用户选中红包的金额==========="+ useHongbao);
			...
			for(Integer hbId :hongbaoId){
				if(hbId != null && hbId > 0){
					...
					if(hb==null){
						System.out.println("===========红包不存在==========="+hbId);
						return 21;//红包不存在
					}
					if(hb.getStatus().compareTo(0) != 0){
						System.out.println("===========红包已使用或已过期==========="+hbId);
						return 23;//红包已使用或已过期
					}
					if(hb.getUserId().compareTo(user.getId()) != 0){
						System.out.println("===========红包不属于投资人==========="+hbId);
						return 21;//红包不属于投资人
					}
				}
			}
		...
		// 账户可用金额少于投标金额
		if (userAccount.getAbleMoney().doubleValue()+Double.valueOf(useHongbao.toString()) < Double.parseDouble(borrowTender.getMoney())) {
			logPledge.debug("【月标结束】"+user.getUsername()+"投标失败,余额不足["+Double.parseDouble(borrowTender.getMoney())+"]");
			return 2;// 余额不足
		}
		...
		if(analysList != null && analysList.size() >0){
			UserAnalys analys = analysList.get(0);
				...
				//android回调
				//if(analys.getPlaceChilderId() != null && analys.getPlaceChilderId().compareTo(62)==0 &&  StringUtils.isNotEmpty(analys.getImei())){
				if(analys.getPlaceChilderId() != null  &&  StringUtils.isNotEmpty(analys.getImei())){
					iosIdfaDao.tenderBack(analys.getImei(),1);
				}
		}
