...
@Component("userAccountDetailCountMulitDirective")
public class UserAccountDetailCountMulitDirective implements TemplateDirectiveModel{
	public static final String TAG_NAME = "userAccountDetailSum_by_multi_type_count";
	private static final String TYPES_PARAMETER_NAME = "types";
	private static final String USERID_PARAMETER_NAME = "userid";
	private static final String COUNT_TYPES_PARAMETER_NAME = "countTypes";
	...
	public void execute(Environment env, Map params, TemplateModel[] loopVars, TemplateDirectiveBody body) throws TemplateException, IOException {
		...
		String countTypes = DirectiveUtil.getStringParameter(COUNT_TYPES_PARAMETER_NAME, params);
		...
		if (StringUtils.isNotEmpty(types)) {
			...
			for(int i =0;i<ts.length;i++) {
				...
				if (i==0) {
					...
					if ("add".equals(cType[i-1])) {
						sum = sum.add(sums[i]);
					} else if ("sub".equals(cType[i-1])) {
