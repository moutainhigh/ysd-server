...
public class PageFlipDirective implements TemplateDirectiveModel {
	...
	public void execute(Environment env, Map params, TemplateModel[] loopVars, TemplateDirectiveBody body) throws TemplateException, IOException {
		...
		if (pager != null) {
			...
			if (pageCount <= 11) {// 总页数小于11，显示全部
				startPageItemNumber = 1;
				endPageItemNumber = pageCount;
			} else if(pageNumber <=6){// 当前页未头部6条，显示前面11条
				startPageItemNumber = 1;
				endPageItemNumber = 11;
			} else if(pageCount - pageNumber <6){// 当前页为尾部6条，显示最后11条
				startPageItemNumber = pageCount-10;
				endPageItemNumber = pageCount;
			} else {//属于显示中间项(头部，尾部有不显示的项)
				startPageItemNumber = pageNumber - 5;
				endPageItemNumber = pageNumber + 5;
			}
