...
import com.qmd.util.bean.BaoPaymentInfo;
...
//    public static String gopay_gateway = ConstantUtil.BAOFOO_WEB + "/PayReceive/bankpay.aspx";
    /**
     * 测试接收地址: https://tgw.baofoo.com/apipay/sdk
		正式环境域名为：https://gw.baofoo.com/apipay/sdk
     */
     public static String gopay_gateway = "https://gw.baofoo.com/apipay/sdk";
...
//			String phone = request.getParameter("phone");// 银行卡号
		    ...
		    String PageUrl=PartnerConfig.URL_RETURN;//页面跳转地址
			String ReturnUrl= PartnerConfig.NOTIFY_URL;//底层访问地址
			...
			try {
				...
				BaoPaymentInfo payInfo = new BaoPaymentInfo();
				payInfo.setVersion(version);
				payInfo.setInput_charset(input_charset);
				payInfo.setLanguage(language);
				payInfo.setData_type(data_type);
				payInfo.setMember_id(MemberID);
				payInfo.setTerminal_id(TerminalID);
				payInfo.setTxn_type(txn_type);
				payInfo.setTxn_sub_type(txn_sub_type);
				payInfo.setData_content(data_content);
				payInfo.setBack_url(ConstantUtil.WEB_SITE+"/userCenter/toBankInput.do") ;
...
//				System.out.println("[解密报文]:" + SecurityUtil.Base64Decode(responseStr));
		 ...
		 *            请求方保留域 长度不超过 128 位
		 */
		private String sendSMS(RechargeConfig rechargeConfig,UserAccountRecharge userAccountRecharge, String PageUrl,String ReturnUrl,String trade_date,String acctName,String idNo,String cardNo,String bankId,String phone) {
...
//			User user = userService.getById(userAccountRecharge.getUserId(), new User());
