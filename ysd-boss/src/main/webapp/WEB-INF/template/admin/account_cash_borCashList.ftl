...
<body class="list">
	...
	<div class="body">
	<form id="listForm" action="account_cash!borrowCashList.action" method="post">
		<input type="hidden" name ="accountCash.user.typeId" id="typeId" value="1"/>
	<div class="listBar">
<#--			<label>用户类型：</label>
				<select name ="accountCash.user.typeId" id="typeId" >
				...
				<#list pager.result as accountCash>
					<tr>
						...
						<#if accountCash.status==0 || accountCash.status==4>  
							<a href="account_cash!edit.action?id=${accountCash.id}" title="[审核]">[审核]</a>
