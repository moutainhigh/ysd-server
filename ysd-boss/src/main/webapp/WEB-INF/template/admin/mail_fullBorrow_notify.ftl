		...
		<div style="padding:30px 50px 40px 40px;">
			<ul>
				<li style=" margin-bottom:44px; padding-left:18px;">
					...
					尊敬的${(borrowDetail.user.username)!"客户"}，您好！<br/>
					您于${borrowDetail.createDate?string("yyyy-MM-dd HH:mm:ss")}投的标【<a href="${websiteurl}/borrow/detail.do?bId=${borrowDetail.borrow.id}">${borrowDetail.borrow.name}</a>】已满，<br/>
					&nbsp;&nbsp;投标金额:${borrowDetail.money?string.currency} <br/>
					&nbsp;&nbsp;通过金额:${borrowDetail.account?string.currency}<br/>
					&nbsp;&nbsp;借款利息:${borrowDetail.interest?string.currency}<br/>
					管理员已于${date?string("yyyy-MM-dd HH:mm:ss")}审核成功，现正在还款，请注意查看。
					</span>
				</li>
			</ul>
...
<div style="background:#ececec;padding:35px;">
	...
	<tr>
		<th valign="middle" style="height:25px;color:#fff; font-size:14px;line-height:25px; font-weight:bold;text-align:left;padding:15px 35px; border-bottom:1px solid #467ec3;background:#810000;border-radius:5px 5px 0 0;">
			<img style="float:left;margin-right:8px;" src="${setting.shopImgContext}${logoPath}"/>
		</th>
	</tr>
	<tr><td>
			<div style="margin:0;padding:35px 35px 40px;">	
				<h2 style="font-weight:bold; font-size:14px;margin:5px 0;">尊敬的${(borrowDetail.user.username)!"客户"}：<span id="mailUserName"></span></h2>
				<p style="color:#313131;line-height:28px;font-size:14px;margin:20px 0;text-indent:2em;">
					您于${borrowDetail.createDate?string("yyyy-MM-dd HH:mm:ss")}投的标【<a href="${websiteurl}/borrow/detail.do?bId=${borrowDetail.borrow.id}">${borrowDetail.borrow.name}</a>】已满, 
				</p>
				<p style="color:#313131;line-height:18px;font-size:14px;text-indent:4em;">
					投标金额:${borrowDetail.money?string.currency} 
				</p>
				<p style="color:#313131;line-height:18px;font-size:14px;text-indent:4em;">
					通过金额:${borrowDetail.account?string.currency}
				</p>
				<p style="color:#313131;line-height:18px;font-size:14px;text-indent:4em;">
					借款利息:${borrowDetail.interest?string.currency}
				</p>
	...
	<tr><td align="center">借款协议号：${agreementId} &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;借款人：${borrow.user.username}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;出借人：详见本协议第一条 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;签订日期：<#if borrow.type==1>${borrow.successTime?string("yyyy-MM-dd ")}<#elseif borrow.type==4>${borrow.successTime?string("yyyy-MM-dd ")}<#else>${lastRdate}</#if></td></tr>
		...
		<td align="center">
			<div>
				<table cellspacing="0" rules="all" border="1" id="ctl00_ContentPlaceHolder1_gvLoans" style="width:98%;border-collapse:collapse;">
                	...
                	<tr>
						<td align="center">${borrowDetail.user.username}</td>
						<td align="center">￥${borrowDetail.account}元</td>
					   	<td align="center">${borrow.timeLimit}<#if borrow.type==4>月<#else>天</#if></td>
                        ...
                        <td align="center">${lastRdate}</td>
						<td align="center">￥${borrowDetail.repaymentAccount}</td>
					</tr>	
			  	</table>
			<div>
		</td>
