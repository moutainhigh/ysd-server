...
<body class="list">
	<div class="bar">
	<#if rechargeInterfaceId!>线下充值<#else>
		充值记录</#if>列表&nbsp;总记录数: ${pager.totalCount} (共${pager.pageCount}页)
	</div>
	<div class="body">
		<form id="listForm" action="user_account_recharge!<#if rechargeInterfaceId!>verifyList<#else>list</#if>.action" method="post">
			...
			<table id="listTable" class="listTable">
				...
				<#list pager.result as userAccountRecharge>
					<tr>
						...
						<#--
						<td>
							<#if userAccountRecharge.compareStatus!>
								<#if userAccountRecharge.compareStatus==0>
									补单
								<#elseif userAccountRecharge.compareStatus==1>
									成功
								<#elseif userAccountRecharge.compareStatus==2>
									未完结成功(两边都是处理中)
								<#elseif userAccountRecharge.compareStatus==3>
									异常数据(本地成功，接口非成功)
								<#elseif userAccountRecharge.compareStatus==4>
									失败(本地有数据，接口无数据)
								</#if>
							</#if>
						</td>-->
						<td>
							...
							<#if rechargeInterfaceId!><!--仅限"线下充值审核"-->
								<a href = "user_account_recharge!edit.action?id=${(userAccountRecharge.id)!}&rechargeInterfaceId=0"><font color="red">审核</font></a>
							<#else>
