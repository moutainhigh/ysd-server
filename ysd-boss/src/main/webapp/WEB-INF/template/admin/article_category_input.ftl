...
<body class="input">
	...
	<div class="body">
		<form id="validateForm" action="<#if isAddAction>article_category!save.action<#else>article_category!update.action</#if>" enctype="multipart/form-data"  method="post" >
			...
			<table class="inputTable">
				...
				<tr>
					...
					<td>
						...
						<#if !isAddAction>
							<#if articleCategory.img!><a href="${imgUrl}/${articleCategory.img!}" target ="_blank">查看</a><#else><font color="red">没有上传图片;</font></#if>
							&nbsp;&nbsp;&nbsp;<@checkbox name="isCleanImg" value="false" />清除图片
						</#if>
					</td>
				</tr>
				...
				<tr>
					...
					<td>
						<input type="text" name="articleCategory.url" class="formText" value="${(articleCategory.url)!}"/>建议以"http://"开头
					</td>
				</tr>
				<tr>
					<th>
					...
					<td>
						<@checkbox name="articleCategory.isEnabled" value="${(articleCategory.isEnabled)!true}" />启用
					</td>
				</tr>
				<tr>
					<th>
