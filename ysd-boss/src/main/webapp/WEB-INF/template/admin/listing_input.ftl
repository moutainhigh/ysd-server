...
<body class="input">
	<div class="bar">
		<#if isAddAction>添加分类<#else>编辑分类</#if>
	</div>
	...
	<div class="body">
		<form id="validateForm" action="<#if isAddAction>listing!save.action<#else>listing!update.action</#if>" method="post">
					...
					<td>
						<#if isAddAction>
							<select name="parentId">
								...
								<#list listingTreeList as listingTree>
									<#if listingTree.grade == 0>
										<option value="${listingTree.id}">
											${listingTree.name}
										</option>
									</#if>
								</#list>
