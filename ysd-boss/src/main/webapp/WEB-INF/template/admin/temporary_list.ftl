...
<body class="list">
	<div class="bar">
		线上充值数据核对列表&nbsp;总记录数: <span id="total_span">${pager.totalCount}</span> (共${pager.pageCount}页)
	</div>
	<div class="body">
		<form id="listForm" action="temporary!list.action" method="post">
			<div class="listBar">
				<input type="button" class="formButton" onclick="location.href='temporary!input.action'" value="导入数据" hidefocus />
			...
			<table id="listTable" class="listTable">
				<tr>
					...
					<th>
						<a href="#" class="" name="status" hidefocus>平台状态</a>
					</th>
					<th>
						<a href="#" class="" name="money" hidefocus>平台金额</a>
					</th>
					<th>
						<a href="#" class=""  hidefocus>状态核对结果</a>
					</th>
					<th>
						<a href="#" class=""  hidefocus>金额核对结果</a>
					</th>
					<th>
						<a>状态</a>
					</th>
				...
				<#list pager.result as u>
					...
					<tr <#if tg_flag> class = "tgsh_class" </#if>>
						...
						<td>
							${m1}  <#if m0==0>第三方金额:${u.temporary.money?string.currency}；注：如果通过，将以平台数据为准</#if>
						</td>
						<td><#if tg_flag>
								已通过
							</#if>
