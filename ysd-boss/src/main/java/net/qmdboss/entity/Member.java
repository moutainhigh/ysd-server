...
import net.qmdboss.bean.Gender;
...
public class Member extends BaseEntity {
	...
	// 获取地区
	@Transient
	public Areashop getArea() {
		if (StringUtils.isEmpty(areaStore)) {
			return null;
		}
		return JsonUtil.toObject(areaStore, Areashop.class);
	}
	
	// 设置地区
	@Transient
	public void setArea(Areashop area) {
		if (area == null) {
			areaStore = null;
			return;
		}
	...
	public void setMemberAttributeValue(MemberAttribute memberAttribute, Object memberAttributeValue) {
		...
		if (systemAttributeType != null) {
			if (systemAttributeType == SystemAttributeType.name) {
				...
				if (memberAttributeValue instanceof Areashop) {
					setArea((Areashop) memberAttributeValue);
				} else {
					throw new IllegalArgumentException("memberAttributeValue type error");
				}
