...
public class AlipayPartner extends BasePaymentProduct {
	...
	public Map<String, String> getParameterMap(PaymentConfig paymentConfig, String paymentSn, BigDecimal paymentAmount, HttpServletRequest httpServletRequest) {
		...
		String notify_url = SettingUtil.getSetting().getShopUrl() + "/shop/payment_notify/" + paymentSn + ".htm";// 消息通知URL
		...
		String return_url = SettingUtil.getSetting().getShopUrl() + "/shop/payment_return/" + paymentSn + ".htm";// 回调处理URL
	...
	public boolean verifySign(PaymentConfig paymentConfig, HttpServletRequest httpServletRequest) {
		...
		String receive_phone = httpServletRequest.getParameter("receive_phone");
		...
		parameterMap.put("receive_phone", receive_phone);
