...
import net.qmdboss.dao.ArticleCategoryDao;
...
public class ArticleCategoryDaoImpl extends BaseDaoImpl<ArticleCategory, Integer> implements ArticleCategoryDao {
	...
	public String findChildListingByKeyValue(String sign,String keyValue){
		Criteria criteria = getSession().createCriteria(ArticleCategory.class);
		...
		criteria.add(Restrictions.eq("parent", getArticleCategoryBySign(sign)));
		String name="";
		if(criteria.list()!= null && criteria.list().size()>0){
			name=((ArticleCategory)criteria.list().get(0)).getName();
		}
		return name;
	}
	...
	private void fillArticleCategory(ArticleCategory articleCategory) {
		ArticleCategory parent = articleCategory.getParent();
		if (parent == null) {
			articleCategory.setPath(articleCategory.getId().toString());
		} else {
