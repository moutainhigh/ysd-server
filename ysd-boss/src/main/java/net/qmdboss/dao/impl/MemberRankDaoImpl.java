...
import net.qmdboss.dao.MemberRankDao;
...
public class MemberRankDaoImpl extends BaseDaoImpl<MemberRank, Integer> implements MemberRankDao {
	...
	public Integer save(MemberRank memberRank) {
		if (memberRank.getIsDefault()) {
			String hql = "from MemberRank as memberRank where memberRank.isDefault = :isDefault";
			List<MemberRank> memberRanks = getSession().createQuery(hql).setParameter("isDefault", true).list();
			if (memberRanks != null) {
				for (MemberRank r : memberRanks) {
