...
import net.qmdboss.entity.Deposit.DepositType;
import net.qmdboss.entity.MemberAttribute.AttributeType;
import net.qmdboss.entity.MemberAttribute.SystemAttributeType;
import net.qmdboss.service.DepositService;
import net.qmdboss.service.MemberAttributeService;
import net.qmdboss.service.MemberRankService;
...
public class MemberAction extends BaseAdminAction {
	...
	private net.qmdboss.service.AreashopService areashopService;
	...
	public String save() {
		...
		for (MemberAttribute memberAttribute : memberAttributeList) {
			...
			Integer memberAttributeValue = memberAttributeValues == null || memberAttributeValues.length == 0 ? null : Integer.parseInt(memberAttributeValues[0]);
			...
			if (systemAttributeType != null) {
				if ( memberAttributeValue != null) {
					if (systemAttributeType == SystemAttributeType.area) {
						Areashop area = areashopService.get(memberAttributeValue);
						if (area == null) {
				...
				if (attributeType == AttributeType.checkbox) {
					...
					if ( memberAttributeValue != null) {
						if (attributeType == AttributeType.number) {
							Matcher matcher = numberPattern.matcher(memberAttributeValue.toString());
							if (!matcher.matches()) {
								addActionError(memberAttribute.getName() + "只允许输入数字!");
								return ERROR;
							}
						} else if (attributeType == AttributeType.alphaint) {
							Matcher matcher = alphaintPattern.matcher(memberAttributeValue.toString());
							if (!matcher.matches()) {
