...
//		net.qmdboss.entity.Setting setting = settingService.load(1);
		String watermarkerImagePath = (String)getServletContext().getAttribute("qmd.setting.watermarkImagePath");
		String watermarkPosition = (String)getServletContext().getAttribute("qmd.setting.watermarkPosition");
		Integer watermarkAlpha = (Integer)getServletContext().getAttribute("qmd.setting.watermarkAlpha");
		File watermarkImageFile = new File(ImageUtil.IMG_DIR+watermarkerImagePath);
		File destImageFile = new File(ImageUtil.IMG_DIR+uploadPath);
		ImageUtil.addWatermark(destImageFile, destImageFile, watermarkImageFile,WatermarkPosition.valueOf(watermarkPosition), watermarkAlpha);
		jsonMap.put("error", 0);
//		jsonMap.put("url", getContextPath() + uploadPath);
		jsonMap.put("url", CommonUtil.WEB_IMG + uploadPath);
		
//		String tempEncode = CommonUtil.encodeUrl(uploadPath);
//		jsonMap.put("url", CommonUtil.WEB_IMG_ENCODE_URL + tempEncode);
...
//		String currentUrl = getContextPath() + setting.getImageBrowsePath() + "/" + path;
		String currentUrl = CommonUtil.WEB_IMG + setting.getImageBrowsePath() + "/" + path;
