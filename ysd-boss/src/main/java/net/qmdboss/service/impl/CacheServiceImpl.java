...
public class CacheServiceImpl implements CacheService, ServletContextAware {
	...
	public static final String LISTING_LIST_PAGE_CACHE_GROUP = "listingList";// 分类列表页面缓存Group
	...
	public void flushListingPageCache(HttpServletRequest request) {
		Cache cache = ServletCacheAdministrator.getInstance(servletContext).getCache(request, PageContext.APPLICATION_SCOPE); 
		cache.flushGroup(LISTING_LIST_PAGE_CACHE_GROUP);
	}
