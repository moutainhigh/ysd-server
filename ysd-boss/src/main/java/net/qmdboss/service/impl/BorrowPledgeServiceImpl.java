...
import net.qmdboss.service.BorrowPledgeService;
 ...
 * 
 */
@Service("borrowPledgeServiceImpl")
public class BorrowPledgeServiceImpl extends BaseServiceImpl<Borrow, Integer>
	 ...
	 */
	@Override
	public synchronized int updateBorrowPledgeFull(Borrow borrow, Integer id, Admin admin) {
		
		verifyLog.debug("    [满标审核 开始]["+id+"]["+borrow.getName()+"]");
		...
		if (borrow1.getStatus() != 5) {
			verifyLog.debug("    [满标审核 结束 状态不对]["+id+"]["+borrow1.getStatus()+"]");
			return 1;
		}
		...
		if (borrow.getStatus() == 3) { // 满标审核
			verifyLog.debug("    [满标审核]["+id+"]["+borrow.getStatus()+"]");
			...
			if (listingPointAble!=null && !StringUtils.isNotEmpty(listingPointAble.getKeyValue()) ) {
			...
			PaymentView retBorrow = interestCalUtil.payback(borrow1
					...
					.valueOf(borrow1.getTimeLimit()), borrow1.getBorStages(),2);
			...
			for (int i = retBorrow.getPaymentDetail().length; i > 0; i--) {
				...
				borRepayDetail.setRepaymentTime(CommonUtil.getDateAfter(
						new Date(), Integer.parseInt(retBorrow
								.getPaymentDetail()[i-1].getDateNum())));
			...
			for (BorrowDetail borrowDetail : borrowDetailSet) {
				...
				for (int i = retView.getPaymentDetail().length; i > 0; i--) {
					...
					if ("1".equals(borrow1.getType())) {// type==1 天标时，取得新的利息管理费手续
						listing = listingDao.getListingBySign(ConstantUtil.DEDUCTION_INTEREST_FEE_DAY);
					} else {
						listing = listingDao.load(1473);
					}
...
//				userAccount1 = userAccountDao.loadLock(userAccount1.getId());
				...
				verifyLog.debug("    [满标审核  投资人资金修改  开始]["+id+"]["+userAccount1.getId()+"]["+userAccount1.getTotal()+"]["+userAccount1.getAbleMoney()+"]["+userAccount1.getUnableMoney()+"]");
				...
				if("1".equals(userPointsAble)) {
					userPoints = (int)(userPointsRateTender*waitCapital.doubleValue()*Integer.parseInt(borrow1.getTimeLimit()));
				...
				verifyLog.debug("    [满标审核  投资人资金修改  结束]["+id+"]["+userAccount1.getId()+"]["+userAccount1.getTotal()+"]["+userAccount1.getAbleMoney()+"]["+userAccount1.getUnableMoney()+"]");
				...
				if ("1".equals(borrow1.getAward())) {
					...
					verifyLog.debug("    [满标审核  投资人收取奖金  开始]["+id+"]["+userAccount1.getId()+"]["+userAccount1.getTotal()+"]["+userAccount1.getAbleMoney()+"]["+userAccount1.getUnableMoney()+"]");
					...
					verifyLog.debug("    [满标审核  投资人收取奖金  结束]["+id+"]["+userAccount1.getId()+"]["+userAccount1.getTotal()+"]["+userAccount1.getAbleMoney()+"]["+userAccount1.getUnableMoney()+"]");
			...
			verifyLog.debug("    [满标审核  借款人资金入账  开始]["+id+"]["+userAccount.getId()+"]["+userAccount.getTotal()+"]["+userAccount.getAbleMoney()+"]["+userAccount.getUnableMoney()+"]");
			...
			verifyLog.debug("    [满标审核  借款人资金入账  结束1]["+id+"]["+userAccount.getId()+"]["+userAccount.getTotal()+"]["+userAccount.getAbleMoney()+"]["+userAccount.getUnableMoney()+"]");
			...
			verifyLog.debug("    [满标审核  借款人借款手续费支付  结束2]["+id+"]["+userAccount.getId()+"]["+userAccount.getTotal()+"]["+userAccount.getAbleMoney()+"]["+userAccount.getUnableMoney()+"]");
			...
			if ("1".equals(borrow1.getAward())) {
				...
				verifyLog.debug("    [满标审核  借款人发放奖金  结束3]["+id+"]["+userAccount.getId()+"]["+userAccount.getTotal()+"]["+userAccount.getAbleMoney()+"]["+userAccount.getUnableMoney()+"]");
			...
			verifyLog.debug("    [满标审核 不通过]["+id+"]["+borrow.getStatus()+"]");
			...
			for (BorrowDetail borrowDetail : borrowDetailSet) {
				...
				verifyLog.debug("    [满标审核 不通过  投资人资金回滚  开始]["+id+"]["+userAccount.getId()+"]["+userAccount.getTotal()+"]["+userAccount.getAbleMoney()+"]["+userAccount.getUnableMoney()+"]");
				...
				verifyLog.debug("    [满标审核 不通过  投资人资金回滚  结束]["+id+"]["+userAccount.getId()+"]["+userAccount.getTotal()+"]["+userAccount.getAbleMoney()+"]["+userAccount.getUnableMoney()+"]");
